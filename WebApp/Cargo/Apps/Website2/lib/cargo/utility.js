 ...
 */
function loadjscssfile(filePath, filetype, callback) {
    var func = callback
    // Instanciate the function from the code.
 ...
 */
function includeJavascript(src, id) {
...
function getStyleSheetByFileName(fileName) {
    ...
    if (fileName[0] != "/") {
        ...
        currentUrl = "http://" + server.ipv4 + ":" + server.port + "/Cargo" + fileName
        // Look for href that contain the filename
        var stylesheet = getStyleSheetByUrl(currentUrl)
        if (stylesheet != null) {
            return stylesheet
        }

        currentUrl = "http://" + server.ipv4 + "/Cargo" + fileName
        stylesheet = getStyleSheetByUrl(currentUrl)
        if (stylesheet != null) {
            return stylesheet
        }

        // Try with the hostname.
        currentUrl = "http://" + server.hostName + ":" + server.port + "/Cargo" + fileName
        stylesheet = getStyleSheetByUrl(currentUrl)
        if (stylesheet != null) {
            return stylesheet
        }

        currentUrl = "http://" + server.hostName + "/Cargo" + fileName
        stylesheet = getStyleSheetByUrl(currentUrl)
        if (stylesheet != null) {
            return stylesheet
        }
 ...
 */
function getStyleRuleValue(style, selector, sheetPath) {
 ...
 */
function addStyleString(id, rule) {
    var node = document.createElement('style');
    node.id = "http://" + server.ipv4 + ":" + server.port + "/" + id
    node.innerHTML = rule;
...
// common variable use by encode64 and decode 64.
 ...
 */
function convertDataURIToBinary(uri) {
...
function resizeImage(base64, maxWidth, maxHeight) {
    ...
    if (typeof (maxHeight) === 'undefined') var maxHeight = 1024;
 ...
 */
function getTimeSinceStr(since) {
    ...
    if (passed < 60) {
        ...
        var passedHours = Math.floor(passed / 3600) + " hrs"
        // The minutes.
        ...
        passedDay = Math.floor(passed / 86400) + " days"
        // The hours.
        passedHours = Math.floor((passed % 86400) / 3600) + " hrs"
        // The minutes.
