 ...
 * Sever side code.
 */
function SetRestrictionRole(roleId, action) {
    server.GetSecurityManager().SetRestrictionRole(roleId, action, messageId, sessionId)
}

/**
 * Set a restriction with a given role (by id) to an action.
 * @param {string} roleId The id of the role to set the restriction to
 * @param {string} action The action to set the restriction on
 * @param {function} successCallback The function to execute in case of success
 * @param {function} errorCallback The function to execute in case of error.
 * @param {object} caller A place to store object from the request context and get it back from the response context.
 */
SecurityManager.prototype.setRestrictionRole = function (roleId, action, successCallback, errorCallback, caller) {
    // server is the client side singleton.
    var params = []
    params.push(createRpcData(roleId, "STRING", "roleId"))
    params.push(new RpcData({ "name": "action", "type": 4, "dataBytes": utf8_to_b64(action.stringify()) }))

    // Call it on the server.
    server.executeJsFunction(
        SetRestrictionRole.toString(), // The function to execute remotely on server
        params, // The parameters to pass to that function
        function (index, total, caller) { // The progress callback
            // Nothing special to do here.
        },
 ...
 * Sever side code.
 */
function RemoveRestrictionRole(roleId, action) {
    server.GetSecurityManager().RemoveRestrictionRole(roleId, action, messageId, sessionId)
}

/**
 * Remove a role (by Id) from the restriction associated to an action
 * @param {string} roleId The id of the role to remove the restriction from
 * @param {string} action The action to remove the restriction
 * @param {function} successCallback The function to execute in case of success
 * @param {function} errorCallback The function to execute in case of error.
 * @param {object} caller A place to store object from the request context and get it back from the response context.
 */
SecurityManager.prototype.removeRestrictionRole = function (roleId, action, successCallback, errorCallback, caller) {
    // server is the client side singleton.
    var params = []
    params.push(createRpcData(roleId, "STRING", "roleId"))
    params.push(new RpcData({ "name": "action", "type": 4, "dataBytes": utf8_to_b64(action.stringify()) }))
    ...
    // Call it on the server.
    server.executeJsFunction(
        RemoveRestrictionRole.toString(), // The function to execute remotely on server
        params, // The parameters to pass to that function
        function (index, total, caller) { // The progress callback
            // Nothing special to do here.
        },
 ...
 * Sever side code.
 */
function RoleHasRestrictionForAction(roleId, action) {
    var roleHasRestrictionForAction = false
    roleHasRestrictionForAction = server.GetSecurityManager().RoleHasRestrictionForAction(roleId, action, messageId, sessionId)
    return roleHasRestrictionForAction
}

/**
 * Verify if a role has a restriction for a given action
 * @param {string} roleId The id of the role to verify the restriction
 * @param {string} action The action to verify the restriction
 * @param {function} successCallback The function to execute in case of success
 * @param {function} errorCallback The function to execute in case of error.
 * @param {object} caller A place to store object from the request context and get it back from the response context.
 */
SecurityManager.prototype.roleHasRestrictionForAction = function (roleId, action, successCallback, errorCallback, caller) {
    // server is the client side singleton.
    var params = []
    params.push(createRpcData(roleId, "STRING", "roleId"))
    params.push(new RpcData({ "name": "action", "type": 4, "dataBytes": utf8_to_b64(action.stringify()) }))
    ...
    // Call it on the server.
    server.executeJsFunction(
        RoleHasRestrictionForAction.toString(), // The function to execute remotely on server
        params, // The parameters to pass to that function
        function (index, total, caller) { // The progress callback
            // Nothing special to do here.
        },
