...
func generateGoMethodCode(attribute *XML_Schemas.CMOF_OwnedAttribute, owner *XML_Schemas.CMOF_OwnedMember, isAssociation bool, packName string) string {
	...
	if owner != nil {
		...
		if isPrimitive || enumMap[typeName] != nil {
			...
			setterStr += "	this.SetNeedSave(true)\n"
		}
		setterStr += "}\n"
	}
	...
	if attribute.Upper == "*" {
		...
		if owner != nil {
			...
			if isPrimitive {
				appendStr += "	if this.M_" + attributeName + "== nil {\n"
				appendStr += "		this.M_" + attributeName + " = make([]" + typeName + ", 0)\n"
				appendStr += "	}\n\n"
				...
				appendStr += "	}\n"
				appendStr += "	if this.M_" + attributeName + "== nil {\n"
				appendStr += "		this.M_" + attributeName + " = make([]string, 0)\n"
				appendStr += "	}\n\n"
			...
			appendStr += "	this.SetNeedSave(true)\n"
...
func generateGoInterfaceCode(packageId string, class *XML_Schemas.CMOF_OwnedMember) {
	...
	classStr += "	/**\n"
	classStr += "	 * Return the list of reference UUID:FieldName where that entity is referenced.\n"
	classStr += "	 */\n"
	classStr += "	GetReferenced() []string\n\n"

	classStr += "	/**\n"
	classStr += "	 * Set a reference\n"
	classStr += "	 */\n"
	classStr += "	SetReferenced(uuid string, field string)\n\n"

	classStr += "	/**\n"
	classStr += "	 * Remove a reference.\n"
	classStr += "	 */\n"
	classStr += "	RemoveReferenced(uuid string, field string)\n\n"
	...
	classStr += "	/**\n"
	classStr += "	 * Return true if the entity need to be saved.\n"
	classStr += "	 */\n"
	classStr += "	IsNeedSave() bool\n\n"

	classStr += "	/**\n"
	classStr += "	 * Set the needsave value.\n"
	classStr += "	 */\n"
	classStr += "	SetNeedSave(needsave bool)\n\n"
	...
	for i := 0; i < len(classes); i++ {
		...
		if class != nil {
			if Utility.Contains(abstractClassLst, class.Name) == false {
				...
				classStr += "	/** keep track if the entity has change over time. **/\n"
				classStr += "	needSave bool\n"
				classStr += "	/** Keep reference to entity that made use of thit entity **/\n"
				classStr += "	Referenced []string\n"
				...
				classStr += "func (this *" + className + ") SetNeedSave(needSave bool){\n"
				classStr += "	this.needSave=needSave\n"
				classStr += "}\n\n"

				classStr += "func (this *" + className + ") GetReferenced() []string {\n"
				classStr += "	if this.Referenced == nil {\n"
				classStr += "		this.Referenced = make([]string, 0)\n"
				classStr += "	}\n"
				classStr += "	// return the list of references\n"
				classStr += "	return this.Referenced\n"
				classStr += "}\n\n"

				classStr += "func (this *" + className + ") SetReferenced(uuid string, field string) {\n"
				classStr += "	if this.Referenced == nil {\n"
				classStr += "		this.Referenced = make([]string, 0)\n"
				classStr += "	}\n"
				classStr += "	if !Utility.Contains(this.Referenced, uuid+\":\"+field) {\n"
				classStr += "		this.Referenced = append(this.Referenced, uuid+\":\"+field)\n"
				classStr += "	}\n"
				classStr += "}\n\n"

				classStr += "func (this *" + className + ") RemoveReferenced(uuid string, field string) {\n"
				classStr += "	if this.Referenced == nil {\n"
				classStr += "		return\n"
				classStr += "	}\n"
				classStr += "	referenced := make([]string, 0)\n"
				classStr += "	for i := 0; i < len(this.Referenced); i++ {\n"
				classStr += "		if this.Referenced[i] != uuid+\":\"+field {\n"
				classStr += "			referenced = append(referenced, uuid+\":\"+field)\n"
				classStr += "		}\n"
				classStr += "	}\n"
