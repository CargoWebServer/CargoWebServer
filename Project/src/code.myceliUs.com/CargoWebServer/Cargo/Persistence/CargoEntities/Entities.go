...
type XsdEntities struct {
	...
	M_restrictions	[]*XsdRestriction	`xml:"restrictions,omitempty"`
...
/** Restrictions **/
func (this *Entities) GetRestrictions() []*Restriction{
	return this.M_restrictions
}

/** Init reference Restrictions **/
func (this *Entities) SetRestrictions(ref interface{}){
	this.NeedSave = true
	isExist := false
	var restrictionss []*Restriction
	for i:=0; i<len(this.M_restrictions); i++ {
		if this.M_restrictions[i].GetUUID() != ref.(*Restriction).GetUUID() {
			restrictionss = append(restrictionss, this.M_restrictions[i])
		} else {
			isExist = true
			restrictionss = append(restrictionss, ref.(*Restriction))
		}
	}
	if !isExist {
		restrictionss = append(restrictionss, ref.(*Restriction))
	}
	this.M_restrictions = restrictionss
}

/** Remove reference Restrictions **/
