...
type XSD_Group struct {
	 ...
	 * Maximum number of occurrences. Note that this value is fixed to one.
	 */
	MaxOccurs uint32 `xml:"maxOccurs,attr"`

	/**
	 * Minimum number of occurrences. Note that this value can be only zero or one.
	 */
	MinOccurs uint32 `xml:"minOccurs,attr"`
...
// Here I will keep the map of element, complex and simple type...
var elementsNameMap = make(map[string]*XSD_Element)
var elementsTypeMap = make(map[string]*XSD_Element)
var complexTypesMap = make(map[string]*XSD_ComplexType)
var simpleTypesMap = make(map[string]*XSD_SimpleType)
var xsdPrimitiveTypesMap = make(map[string]string)
var substitutionGroupMap = make(map[string][]string)
...
// Those have the same structure thant other xml type but with
// a different name...
var aliasElementType = make(map[string]string)

/**
 * Init the maps...
 */
func loadSchema(schema *XSD_Schema) {
	...
	for i := 0; i < len(schema.Elements); i++ {
		...
		if schema.Elements[i].IsAbstract == "true" {
			if contains(superClassesLst, elementType) == false {
		...
		if len(schema.Elements[i].SubstitutionGroup) > 0 {
			...
			if contains(superClassesLst, substitutionGroupType) == false {
			...
			if contains(substitutionGroupMap[substitutionGroupType], elementType) == false {
	...
	for i := 0; i < len(schema.ComplexTypes); i++ {
		...
		//log.Println("Class -------------->", elementType)
		//log.Println(schema.ComplexTypes[i])
		//log.Println("Element Name -------------->", schema.ComplexTypes[i].Name)
