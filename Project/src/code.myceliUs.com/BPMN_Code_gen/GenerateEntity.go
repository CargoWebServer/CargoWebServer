...
func generateEntity(packageId string) {
	...
	imports = append(imports, "code.google.com/p/go-uuid/uuid")
	imports = append(imports, "code.myceliUs.com/CargoWebServer/Cargo/Utility")
	...
	for i := 0; i < len(classes); i++ {
		...
		if class != nil {
			if contains(abstractClassLst, class.Name) == false {
				...
				if contains(superClassesLst, class.Name) == true {
					className += "_impl"
				}
				...
				for i := 0; i < len(imports_); i++ {
					if !contains(imports, imports_[i]) {
 ...
 */
func generateNewEntityFunc(packageId string, class *CMOF_OwnedMember) string {
	...
	entityConstructorStr += "		}\n"
	entityConstructorStr += "	}else{\n"
	entityConstructorStr += "		uuidStr = \"" + packageId + "." + class.Name + "%\" + uuid.NewRandom().String()\n"
	entityConstructorStr += "	}\n"
...
func generateRegisterObjects(packageId string) string {
	...
	for i := 0; i < len(classes); i++ {
		...
		if class != nil {
			if contains(abstractClassLst, class.Name) == false {
				className := class.Name
				if contains(superClassesLst, class.Name) {
					className += "_impl"
				}
...
func generateNewEntityFromObject(packageId string, class *CMOF_OwnedMember) string {
...
func generateAppendRefFunc(packageId string, class *CMOF_OwnedMember) string {
...
func generateAppendChildFunc(packageId string, class *CMOF_OwnedMember) string {
...
func generateEntityAttribute(attribute *CMOF_OwnedAttribute, packName string, prototypeName string, index *int, isAssociation bool, isId bool, isIndex bool) string {
	...
	*index++
	...
	//isInterface := contains(superClassesLst, typeName)
...
func generateEntityAttributes(class *CMOF_OwnedMember, packageId string, prototypeVar string, index *int) string {
...
func generateEntityAssociations(class *CMOF_OwnedMember, packageId string, prototypeVar string, index *int) string {
	// The super classes code...
	classStr := ""
 ...
 */
func generateEntityPrototypeFunc(packageId string, class *CMOF_OwnedMember) string {
	...
	if contains(abstractClassLst, class.Name) {
...
func generateEntityQueryFields(class *CMOF_OwnedMember, packageId string) string {
	// The super classes code...
	classStr := ""
...
func IsRef(attribute *CMOF_OwnedAttribute) bool {
...
func generateEntityInitAttribute(class *CMOF_OwnedMember, attribute *CMOF_OwnedAttribute, packName string, prototypeName string, index *int, isAssociation bool) string {
	var memberStr string
	...
	if isPrimitive && memberTypeName != "interface{}" {
		...
		if attribute.Upper == "*" {
			if isRef || isAssociation {
				...
				if contains(abstractClassLst, typeName) {
					...
					for i := 0; i < len(implementations); i++ {
						impl := ""
						if contains(superClassesLst, implementations[i]) {
							impl = "_impl"
						}
						if i == 0 {
					...
					if contains(superClassesLst, typeName) {
						memberStr += "				/** abstract class **/\n"
 ...
 */
func generateEntityInitFunc(packageId string, class *CMOF_OwnedMember) string {
	impl := ""
	if contains(superClassesLst, class.Name) {
		impl = "_impl"
	}
...
func generateEntitySaveEntityInfo(class *CMOF_OwnedMember, attribute *CMOF_OwnedAttribute, packageId string, isAssociation bool) string {
	entityInfoStr := ""
	...
	if isPrimitive {
		...
		if attribute.Upper == "*" {
			...
			if !isRef {
				...
				if contains(abstractClassLst, typeName) || contains(superClassesLst, typeName) {
 ...
 */
func generateEntitySaveFunc(packageId string, class *CMOF_OwnedMember) string {
 ...
 */
func generateEntityDeleteFunc(packageId string, class *CMOF_OwnedMember) string {
 ...
 * Generate the entity delete function of a given class.
 */
func generateEntityExistsFunc(packageId string, class *CMOF_OwnedMember) string {
