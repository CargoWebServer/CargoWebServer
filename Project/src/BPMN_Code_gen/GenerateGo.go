...
func generateGoXmlParser(class *CMOF_OwnedMember, packName string) string {
	xmlParserStr := ""
...
func generateGoXmlEnumParser(member *CMOF_OwnedMember, packName string) string {
...
func generateGoXmlParserContent(class *CMOF_OwnedMember, element *XSD_Element, packName string) string {
			...
			if len(baseType) > 0 {
				...
				if len(choice.Sequence.Elements) > 0 {
					for j := 0; j < len(choice.Sequence.Elements); j++ {
						element := choice.Sequence.Elements[j]
						xmlParserContentStr += generateGoXmlElement(&element, packName)
					}
				}
				if len(choice.Elements.Ref) > 0 {
					xmlParserContentStr += generateGoXmlElement(&choice.Elements, packName)
				}
...
func generateBaseClassXmlMembers(elementName string, packName string, element *XSD_Element, elementType string, classIndex *int, isArray bool, isPointer bool) string {
	elementStr := ""
...
func generateGoXmlElement(element *XSD_Element, packName string) string {
	...
	if len(xsdPrimitiveTypesMap[elementType]) > 0 {
		...
		if contains(abstractClassLst, elementType) == false {
			elementStr += "	M_" + element.Name
			...
			} else if minOccurs == "0" || contains(superClassesLst, elementType) {
				elementStr += "	*" + packName_ + "Xsd" + elementType + "	`xml:\"" + elementType_ + ",omitempty\"`"
			} else {
...
func generateGoXmlAttribute(attribute *XSD_Attribute) string {
	if len(attribute.Name) == 0 {
		return ""
	}
 ...
 */
func generateGoMemberCode(attribute *CMOF_OwnedAttribute, isAssociation bool, packName string) string {
 ...
 */
func generateGoClassMembersCode(class *CMOF_OwnedMember, packageId string) string {
	var classStr string
 ...
 */
func generateGoMethodCode(attribute *CMOF_OwnedAttribute, owner *CMOF_OwnedMember, isAssociation bool, packName string) string {
	...
	if owner != nil {
		...
		if contains(superClassesLst, ownerName) == true {
			ownerName += "_impl"
		}
			...
			if isRef {
				...
				if classesMap[cast] != nil {
					if hasId(classesMap[cast]) {
						...
						if isPointer && !contains(abstractClassLst, cast) {
							cast = "*" + cast
						}
		...
		if (isRef || attribute.IsComposite == "true") && classesMap[typeName_] != nil {
			if hasId(classesMap[typeName_]) {
				...
				if attribute.Upper == "*" {
					...
					if contains(abstractClassLst, typeName_) || contains(superClassesLst, typeName_) {
...
func generateGoInterfacesCode(packageId string) {
	...
	for i := 0; i < len(classes); i++ {
		...
		if class != nil {
			if contains(abstractClassLst, class.Name) == true {
...
func generateGoInterfaceCode(packageId string, class *CMOF_OwnedMember) {
